...
class SixEightLogitsProcessor(LogitsProcessor):

    def __init__(self, rhymable_dict: Dict[int, List[int]], even_ids: List[int], uneven_ids: List[int], input_ids_seq_length: int,
                 unmarked_ids=List[int], grave_ids=List[int],  newline_id=int, all_ids=List[int]):
        self.rhymable_dict = rhymable_dict
        self.even_ids = set(even_ids)
        self.uneven_ids = set(uneven_ids)
        self.unmarked_ids = set(unmarked_ids)
        self.grave_ids = set(grave_ids)
        self.input_ids_seq_length = input_ids_seq_length
        self.newline_id = newline_id
        self.all_ids = set(all_ids)
        # print(set(all_ids) - set(newline_id))
        self.non_newline_ids = list(set(all_ids) - set(newline_id))
        self.pair_length = 16
        self.bias = -float("inf")

    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        curr_idx = input_ids.shape[-1] - self.input_ids_seq_length
        if curr_idx % self.pair_length in [6,15]:
            scores[:, self.non_newline_ids] =  self.bias
        else:
            scores[:, self.newline_id] =  self.bias

        # Phải là vần bằng
        not_allowed_ids = None
        if curr_idx % self.pair_length in [1,5,8, 12, 14]:
            not_allowed_ids = list(self.all_ids - self.even_ids)
        elif curr_idx % self.pair_length in [3, 10]:
            not_allowed_ids = list(self.all_ids - self.uneven_ids)
        if not_allowed_ids is not None:
            scores[:, not_allowed_ids] = self.bias

        # từ thứ 6 của câu 8
        for i in range(len(scores)):
            if curr_idx % self.pair_length == 12:
                allowed_ids = self.rhymable_dict.get(int(input_ids[i][curr_idx + self.input_ids_seq_length - 7]), [])
                not_allowed_ids = list(self.all_ids - allowed_ids)
                scores[i, not_allowed_ids] = self.bias

        if curr_idx > self.pair_length:
            for i in range(len(scores)):
                if curr_idx % self.pair_length == 5:
                    allowed_ids = self.rhymable_dict.get(int(input_ids[i][curr_idx +self.input_ids_seq_length - 7]), [])
                    not_allowed_ids = list(self.all_ids - allowed_ids)
                    scores[i, not_allowed_ids] = self.bias

        for i in range(len(scores)):
            if curr_idx % self.pair_length == 14:
                if int(input_ids[i][curr_idx  + self.input_ids_seq_length- 2]) in self.grave_ids:
                    not_allowed_ids = list(self.all_ids - self.unmarked_ids)
                else:
                    not_allowed_ids = list(self.all_ids - self.grave_ids)
